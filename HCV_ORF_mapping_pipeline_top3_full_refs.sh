...
#### Emma Thomson 16/12/2019 updated for fuller refs 17/01/2020
...
#### This version runs tanoti against the top 3 Create kmers hits

echo "HCV mapping pipeline has started - includes reference sequences from the top 3 genotyping hits...
"
...
#TROUBLESHOOTING Need to have All_refs file in the same folder
CREATE_KMERS-FQ-T -i i All_refs.fasta -1 $fq1.fastq -2 $fq2.fastq  -c 1 -p 1 -v >> $fq1.geno
...
#This bit pulls out the top 3 hits
...
fq1=`echo $file|awk '{print $1}' `;
fq2=`echo $file|awk '{print $2}' `;
tanoti -i $fq1.fastq $fq2.fastq -r /home1/Acute_HCV_UK/Scripts/refs_ORF_full/$(less $fq1.newbest).fa  -p 1 -u 0 -o $fq1-$(less $fq1.newbest1|tr -d '\r').sam -m 50
tanoti -i $fq1.fastq $fq2.fastq -r /home1/Acute_HCV_UK/Scripts/refs_ORF_full/$(less $fq1.newbest).fa  -p 1 -u 0 -o $fq1-$(less $fq1.newbest2|tr -d '\r').sam -m 50
tanoti -i $fq1.fastq $fq2.fastq -r /home1/Acute_HCV_UK/Scripts/refs_ORF_full/$(less $fq1.newbest).fa  -p 1 -u 0 -o $fq1-$(less $fq1.newbest3|tr -d '\r').sam -m 50
done < tanlist-$$
...
cat Consensus_90.fasta All_refs.fasta>>Consensus_plus_refs.fasta
