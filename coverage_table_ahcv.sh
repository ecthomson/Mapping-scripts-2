...
grep "sam" tab_$stats | cut -f 1,2 -d "_"> fn1_$stats
...
tac $stats| sed '/Coverage/I,+1 d'| tac | grep sam | cut -f 1,2 -d "_" | sed 's/$/\t 0 \t 0/g' > neg_$stats

#Makes the full table
paste fn1_$stats tab1_$stats >pos_$stats 

cat pos_$stats neg_$stats | sort -k 2n > coverage_table_$stats

#This is the R script bit (file MUST be in the folder)
Rscript coverage_mapped_reads.R coverage_table_$stats
